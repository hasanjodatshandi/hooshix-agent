...
export function registerTaskTools(server: McpServer) {
  ...
  server.registerTool("task_step_risks", { title: "Preview Step Risks", description: "🗂️ TASK (read) — Dry-run risk analysis: which planned steps will require approval before execution.\n\nExample: { \"steps\": [{ \"tool\": \"execute_command\", \"arguments\": { \"command\": \"node\", \"args\": [\"-e\", \"console.log(1)\"] } }] }", annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true }, inputSchema: z.object({ steps: z.array(z.object({ tool: z.string(), arguments: z.record(z.string(), z.unknown()).default({}) })), ...traceSchema }) }, async ({ steps, correlationId }) => {
    assertToolPermission("task_list"); const traceId = resolveCorrelationId(correlationId);
